        ...
        if self.pos != 0:
            ...
            if self.policy.exitOnLastRtnPips > 0:
                if self.pos > 0 and self.entrust == 0 \
                    ...
                    self.writeCtaLog(u'{0},onTick跟随止损，平仓多单{1}手,价格:{2}'.format(tick.datetime, self.inputSS, tick.lastPrice))
                ...
                if self.pos < 0 and self.entrust == 0 \
                    ...
                    self.writeCtaLog(u'{0},onTick跟随止损，平仓空单{1}手,价格:{2}'.format(tick.datetime, self.inputSS, tick.lastPrice))
            ...
            if self.policy.exitOnStopPrice > 0:
                if self.pos > 0 and self.entrust == 0 \
                    ...
                    self.writeCtaLog(u'{0},onTick固定止损，平仓多单{1}手,价格:{2}'.format(tick.datetime, self.inputSS, tick.lastPrice))
                    orderid = self.sell(price=tick.lastPrice, volume=self.inputSS, orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
                ...
                if self.pos < 0 and self.entrust == 0 \
                        and tick.lastPrice > self.policy.exitOnStopPrice:
                    self.writeCtaLog(u'{0},onTick固定止损，平仓空单{1}手,价格:{2}'.format(tick.datetime, self.inputSS, tick.lastPrice))
                    orderid = self.cover(price=tick.lastPrice, volume=self.inputSS, orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
