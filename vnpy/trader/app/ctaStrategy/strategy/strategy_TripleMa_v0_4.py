...
from vnpy.trader.utilSinaClient import UtilSinaClient
...
#----------------------------------------------------------------------
        ...
        if self.position.pos != 0:
            ...
            if self.policy.exitOnLastRtnPips > 0:
                if self.position.pos > 0 and self.entrust == 0 \
                    ...
                    self.writeCtaLog(
                        u'{0},onTick跟随止损，平仓多单{1}手,价格:{2}'.format(tick.datetime, abs(self.position.pos), tick.lastPrice))
                    orderid = self.sell(price=tick.lastPrice, volume=abs(self.position.pos), orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                ...
                if self.position.pos < 0 and self.entrust == 0 \
                    ...
                    self.writeCtaLog(
                        u'{0},onTick跟随止损，平仓空单{1}手,价格:{2}'.format(tick.datetime, abs(self.position.pos), tick.lastPrice))
                    orderid = self.cover(price=tick.lastPrice, volume=abs(self.position.pos), orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
                    return
            ...
            if self.policy.exitOnStopPrice > 0:
                if self.position.pos > 0 and self.entrust == 0 \
                    ...
                    self.writeCtaLog(
                        u'{0},onTick固定止损，平仓多单{1}手,价格:{2}'.format(tick.datetime, abs(self.position.pos), tick.lastPrice))
                    orderid = self.sell(price=tick.lastPrice, volume=abs(self.position.pos), orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
                ...
                if self.position.pos < 0 and self.entrust == 0 \
                        and tick.lastPrice > self.policy.exitOnStopPrice:
                    self.writeCtaLog(
                        u'{0},onTick固定止损，平仓空单{1}手,价格:{2}'.format(tick.datetime, abs(self.position.pos), tick.lastPrice))
                    orderid = self.cover(price=tick.lastPrice, volume=abs(self.position.pos), orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
            ...
            # 海龟加仓
            if self.policy.addPos and self.position.avaliablePos2Add() > 0:
                # 加仓策略使用特定pip间隔（例如海龟的N）
                if self.policy.addPosOnPips > EMPTY_INT:
                    # 根据 ATR更新N
                    self.policy.addPosOnPips = int(self.atr / (2 * self.minDiff))

                # 加多仓
                if self.position.pos > 0 \
                        and self.entrust == 0 \
                        and tick.lastPrice >= (self.policy.entryPrice + self.policy.addPosOnPips * self.minDiff):
                    self.writeCtaLog(u'{0},onTick加仓多单{1}手,价格:{2}'.format(tick.datetime, self.inputSS, tick.lastPrice))
                    orderid = self.buy(price=tick.lastPrice, volume=self.inputSS, orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
                        # 更新开仓价格
                        self.policy.entryPrice = tick.lastPrice
                        # 设置新开仓价-2ATR为止损价
                        self.writeCtaLog(u'更新止损价:{0}->{1}'
                                         .format(self.policy.exitOnStopPrice,
                                                 max(self.policy.exitOnStopPrice,
                                                     self.policy.entryPrice - 2 * self.atr)))
                        self.policy.exitOnStopPrice = max(self.policy.exitOnStopPrice,
                                                          self.policy.entryPrice - 2 * self.atr)
                ...
                if self.position.pos < 0 \
                        and self.entrust == 0 \
                        and tick.lastPrice <= (self.policy.entryPrice - self.policy.addPosOnPips * self.minDiff):
                    self.writeCtaLog(u'{0},onTick加仓空单{1}手,价格:{2}'.format(tick.datetime, self.inputSS, tick.lastPrice))
                    orderid = self.short(price=tick.lastPrice, volume=self.inputSS, orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
                        # 更新开仓价格
    ...
    def onBarM5(self, bar):
        ...
        # 未初始化完成
        if not self.inited:
            if len(self.lineM5.lineBar) > 120 + 10:
                self.inited = True
            else:
                return
        ...
        # 更新ATR
        if len(self.lineM5.lineAtr1) > 1 and self.lineM5.lineAtr1[-1] > 2:
        ...
        if len(self.lineM5.lineMa3) < 1 or len( self.lineM5.lineMa2)<2:
            return
        ...
        # 持仓，检查是否满足平仓条件
            ...
            if self.policy.addPos and self.position.avaliablePos2Add() > 0:
                ...
                if self.position.pos > 0 \
                    ...
                    self.writeCtaLog(u'{0},加仓多单{1}手,价格:{2}'.format(bar.datetime, self.inputSS, bar.close))
                    orderid = self.buy(price=bar.close, volume=self.inputSS, orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
                        ...
                        self.writeCtaLog(u'更新止损价:{0}->{1}'
                                         .format(self.policy.exitOnStopPrice,
                                                 max(self.policy.exitOnStopPrice, self.policy.entryPrice - 2 * self.atr)))
                        self.policy.exitOnStopPrice = max(self.policy.exitOnStopPrice,
                                                          self.policy.entryPrice - 2 * self.atr)
                ...
                if self.position.pos < 0 \
                        and bar.close <= (self.policy.entryPrice-self.policy.addPosOnPips*self.minDiff):
                    self.writeCtaLog(u'{0},加仓空单{1}手,价格:{2}'.format(bar.datetime, self.inputSS, bar.close))
                    orderid = self.short(price=bar.close, volume=self.inputSS, orderTime=self.curDateTime)
                    if orderid:
                        # 更新下单时间（为了定时撤单）
                        self.lastOrderTime = self.curDateTime
    ...
    def __dailyCloseCheck(self, bar):
        ...
        # 强制平仓
        if self.position.pos == self.inputSS and bar.close < self.policy.entryPrice + self.atr and self.entrust ==0:
            self.writeCtaLog(u'强制日内平亏损多仓')
            # 降低两个滑点
            orderid = self.sell(price=bar.close-2*self.minDiff, volume=abs(self.position.pos), orderTime=self.curDateTime)
            if orderid:
                # 更新下单时间
                self.lastOrderTime = self.curDateTime
            return True

        if self.position.pos == (0-self.inputSS) and bar.close > self.policy.entryPrice - self.atr:
            self.writeCtaLog(u'强制日内平亏损空仓')

            orderid = self.cover(price=bar.close+2*self.minDiff, volume=abs(self.position.pos), orderTime=self.curDateTime)
            if orderid:
                # 更新下单时间（为了定时撤单）
                self.lastOrderTime = self.curDateTime
            return True
...
def tRbByTick():
    ...
    # 设置回测用的数据结束日期
    engine.setEndDate('20170330')

    # engine.connectMysql()
    engine.setDatabase(dbName='ticks', symbol='RBMI')
...
def tRbByBar():
    ...
    engine.setRate(float(0.0001))    # 万1
    engine.setSize(10)         # 合约大小
    engine.setMarginRate(0.11)  # 合约的保证金比率
